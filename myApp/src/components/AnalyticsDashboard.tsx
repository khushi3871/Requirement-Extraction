...
const StatCard = ({ icon, title, value, detail, color }: any) => (
  <div className="bg-[#141121] p-6 rounded-2xl border border-[#4729e0]/20 shadow-lg hover:border-[#4729e0]/50 transition-all group">
    <div
      className={`mb-4 p-3 bg-slate-900 w-fit rounded-xl group-hover:scale-110 transition-transform ${color}`}
    >
      {icon}
    </div>
...
const AnalyticsDashboard = ({ projectList = [] }: { projectList: any[] }) => {
  ...
  const hoursSaved = (
    (serverStats.funcCount + serverStats.nonFuncCount) *
    0.5
  ).toFixed(1);

  const distributionData = [
    { name: "Functional", value: serverStats.funcCount || 1, color: "#4729e0" },
    {
      name: "Non-Functional",
      value: serverStats.nonFuncCount || 0,
      color: "#10b981",
    },
  ];
  ...
  const todayStats: any = serverStats.weeklyHistory?.find(
    (d: any) => d._id === todayIndex,
  );
  const todayInputCount = todayStats ? todayStats.totalInputs : 0;
  ...
  return (
    <div className="p-8 bg-[#0f0d19] min-h-screen text-slate-100 overflow-y-auto custom-scrollbar">
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-8 gap-4">
        <div>
          ...
          <p className="text-slate-400 text-sm">
            ...
            <span className="text-[#4729e0] font-bold">{selectedProject}</span>
          </p>
        </div>
        ...
        <ChartContainer title="Domain Split">
          <ResponsiveContainer width="100%" height={250}>
            <PieChart>
              ...
              <Tooltip
                contentStyle={{ backgroundColor: "#141121", border: "none" }}
              />
            </PieChart>
          </ResponsiveContainer>
        </ChartContainer>
          ...
          <ResponsiveContainer width="100%" height={250}>
            <FunnelChart>
              ...
              <Funnel
                dataKey="value"
                data={[
                  { value: 100, name: "Raw Input" },
                  { value: 45, name: "NLP Filter" },
                  { value: 20, name: "Structured" },
                ]}
                isAnimationActive
              >
                <LabelList
      ...
      {/* WEEKLY VELOCITY SECTION - UNIFORM COLOR */}
      <div className="bg-[#141121] p-6 rounded-2xl border border-[#4729e0]/20 shadow-xl">
        ...
        <div className="h-64 w-full flex items-end justify-around gap-2 px-2 pb-2 relative border-b border-slate-800">
          ...
          {weekDays.map((day, index) => {
            ...
            const dayData: any = serverStats.weeklyHistory?.find(
              (d: any) => d._id === dayNum,
            );
            const inputCount = dayData ? dayData.totalInputs : 0;

            // MATH: Scaling based on 50 inputs max
            const barHeight =
              inputCount > 0 ? Math.min((inputCount / 50) * 100, 100) : 5;
            ...
            return (
              ...
              >
                ...
                <div
                  ...
                  style={{
                    height: `${barHeight}%`,
                    backgroundColor: "#4729e0", // Unified brand blue
                    boxShadow:
                      inputCount > 0
                        ? "0 4px 15px rgba(71, 41, 224, 0.3)"
                        : "none",
                    opacity: inputCount > 0 ? 1 : 0.2, // Solid if data, faded nub if 0
                  }}
                />
