...
export default function KnowledgeGraph({ data }: { data: any }) {
  const [logs, setLogs] = React.useState<string[]>(["[System] Initializing MiroFish Engine..."]);
  ...
  React.useEffect(() => {
    ...
    const interval = setInterval(() => {
      setLogs(prev => [phrases[Math.floor(Math.random() * phrases.length)], ...prev].slice(0, 8));
...
${analysis_details.stakeholders?.map((s: string) => `
* Stakeholder: ${s}
    ...
    <div className="h-full w-full bg-[#141121] rounded-2xl border border-slate-800/50 relative overflow-hidden">
      ...
      <div className="absolute top-6 right-6 z-10 w-64 bg-slate-950/80 backdrop-blur-md border border-slate-800 rounded-lg p-3 font-mono text-[10px]">
        <div className="flex items-center gap-2 mb-2 border-b border-slate-800 pb-1">
          ...
          <span className="text-slate-400 uppercase font-bold tracking-widest">Live Simulation</span>
        </div>
        <div className="flex flex-col gap-1.5 h-40 overflow-hidden">
          {logs.map((log, i) => (
            <div key={i} className={`${i === 0 ? 'text-emerald-400' : 'text-slate-500'} transition-all duration-500`}>
              {log}
            </div>
          ))}
        </div>
     ...
     <ForceGraph2D
        ...
        nodeCanvasObject={(node: any, ctx, globalScale) => {
          const label = node.label.replace('Stakeholder: ', '').replace('Req: ', '');
          ...
          if (globalScale > 1.2) {
            ...
            ctx.fillRect(node.x - textWidth/2 - 2, node.y + 10, textWidth + 4, fontSize + 2);
