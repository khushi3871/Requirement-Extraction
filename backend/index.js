...
app.get("/api/auth/jira/callback", async (req, res) => {
  ...
  if (!code) {
    return res
      .status(400)
      .send("Authorization failed: No code received from Jira.");
  }
  ...
  try {
    ...
    console.log(
      "✅ OAuth Success! Access Token Obtained:",
...
`);
    ...
    res
      .status(500)
      .send(
        "Failed to complete the Jira handshake. Check console for details.",
      );
  }
});
...
const mongoURI =
  process.env.MONGODB_URI || "mongodb://127.0.0.1:27017/reqsphere";
mongoose
  .connect(mongoURI)
...
app.listen(PORT, "0.0.0.0", () => {
  ...
  console.log(
    `🔗 Callback listening at http://localhost:${PORT}/api/auth/jira/callback`,
  );
});
